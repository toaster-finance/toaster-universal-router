...
export class TickMath {
  ...
  public static readonly Q32 = 2n ** 32n;
  public static readonly Q96 = 2n ** 96n;
  public static getSqrtRatioAtTick(tick: bigint): bigint {
    if (
      tick >= this.MIN_TICK &&
      tick <= this.MAX_TICK &&
      Number.isInteger(tick)
    )
      throw new Error("tick is not in range");

    const absTick = Number(tick < 0 ? tick * -1n : tick);
    ...
    if (tick > 0) {
      ratio = ethers.constants.MaxUint256.toBigInt() / ratio;
    }

    // back to Q96
    const result =
      ratio / this.Q32 > 0n ? ratio / this.Q32 + 1n : ratio / this.Q32;
