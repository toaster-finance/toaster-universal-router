...
export const getMakingAmount = async (params: IGetMakingAmount): Promise<{ isMakingZero: boolean, makingAmount: bigint, estimateTakingAmount: bigint }> => {
    ...
    console.log(Number(optimalAmount0), Number(optimalAmount1))
    if(_liquidity0 > _liquidity1) {
        ...
        if((sqrtPriceCX96 ** 2n) >> 192n != 0n) {
            ...
            const exchangeRate = Number(sqrtPriceCX96 ** 2n)  >> 192;
            ...
            // const estimateTakingAmount = makingAmount / exchangeRate;
            console.log("2 exchangeRate", exchangeRate);
            // return { isMakingZero, makingAmount, estimateTakingAmount };
        }
    } else {
        ...
        if ((sqrtPriceCX96 ** 2n) >> 192n != 0n) {
          ...
          const exchangeRate = Number(sqrtPriceCX96 ** 2n) >> 192;
            ...
            // const estimateTakingAmount = makingAmount / exchangeRate;
            console.log("3 exchangeRate", exchangeRate);
        //   return { isMakingZero, makingAmount, estimateTakingAmount };
        } else {
          // exchangeRate = (token0 amount) / (token1 amount) = 2 ** 192n / sqrtPriceCX96 ** 2n
          const exchangeRate = (1 << 192) / Number(sqrtPriceCX96 ** 2n);
        //   const makingAmount = (amount0Desired * optimalAmount1 - amount1Desired * optimalAmount0) / (optimalAmount0 + optimalAmount1 * exchangeRate);
        //     const estimateTakingAmount = makingAmount * exchangeRate;
            console.log("4 exchangeRate", exchangeRate);
        //   return { isMakingZero, makingAmount, estimateTakingAmount };
        }
    }
