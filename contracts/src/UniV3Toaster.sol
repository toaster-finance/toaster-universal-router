...
contract UniV3Toaster is IPostInteractionNotificationReceiver{
    ...
    struct PositionCache {
        address baseToken;
        address quoteToken;
        uint256 baseAmountDesired; // baseAmountDesired: 투자하려는 토큰의 양 -> 결과적으로 baseAmountDesired - makingAmount
        uint256 quoteAmountDesired; // quoteAmountDesired: 투자하려는 토큰의 양 -> 결과적으로 quoteAmountDesired + (takingAmount)
        uint24 fee;
        int24 tickLower;
        int24 tickUpper;
    } 
    ...
    function fillOrderPostInteraction(bytes32 orderHash, address maker, address, uint256 makingAmount, uint256 takingAmount, uint256 remainingAmount, bytes memory interactionData) override external {
        ...
        uint surplus = calculateActualAmount(position, actual, takingAmountTotal);
        ...
        manager.mint(INonfungiblePositionManager.MintParams({
            token0: position.baseToken,
            token1: position.quoteToken,
            fee: position.fee,
            tickLower: position.tickLower,
            tickUpper: position.tickUpper,
            amount0Desired: actual.baseAmount,
            amount1Desired: actual.quoteAmount,
            amount0Min: 0,
            amount1Min: 0,
            recipient: maker,
        ...
        if(surplus > 0 && position.quoteToken != address(WETH)) {
            // if surplus > 0, return surplus to maker , it will be always quoteToken
            SafeERC20.safeTransfer(IERC20(position.quoteToken), maker, surplus);
        } else if(surplus > 0 && position.quoteToken == address(WETH)){
            unwrapWETH9(surplus, maker);
        }
        
    }
    function calculateActualAmount(PositionCache memory position, ActualAmountCache memory actual,uint takingAmountTotal) internal view returns(uint surplus) {
        ...
        if( _baseAmount > actual.baseAmount ) {
            ...
            // calculate quoteAmount from baseAmountDesired
            actual.quoteAmount = getOppTokenAmount(position.tickLower, position.tickUpper, position.baseToken , position.quoteToken, position.fee, position.baseAmountDesired);
            surplus = actual.quoteAmount > takingAmountTotal ? 0: takingAmountTotal - actual.quoteAmount;
    ...
    function getOppTokenAmount(int24 tickLower, int24 tickUpper,address self, address opp, uint24 fee, uint256 selfAmount) internal view returns(uint256 oppAmount){
        IUniswapV3PoolState _pool = IUniswapV3PoolState(factory.getPool(self, opp, fee));

        (uint160 sqrtRatioCX96,,,,,,) = _pool.slot0();
        ...
        // uint128 _liquidity;
        // (_liquidity, oppAmount) = (LiquidityAmounts.getLiquidityForAmount0(sqrtRatioLX96, sqrtRatioUX96, selfAmount), SqrtPriceMath.getAmount1Delta(sqrtRatioCX96, sqrtRatioUX96,_liquidity ,true)):(LiquidityAmounts.getLiquidityForAmount1(sqrtRatioLX96, sqrtRatioUX96, selfAmount),SqrtPriceMath.getAmount0Delta(sqrtRatioLX96, sqrtRatioCX96, _liquidity,true));

        uint128 _liquidity = isSelfZero ? LiquidityAmounts.getLiquidityForAmount0(sqrtRatioLX96, sqrtRatioUX96, selfAmount): LiquidityAmounts.getLiquidityForAmount1(sqrtRatioLX96, sqrtRatioUX96, selfAmount);
    ...
    function getMakingAmount(int24 tickLower, int24 tickUpper, address token0, address token1, uint24 fee, uint256 amount0Desired, uint256 amount1Desired) external view returns(bool isZeroForOne, uint256 makingAmount){
 
        require(token0 < token1 && amount0Desired > 0 && amount1Desired > 0, "INVALID_PARAMS");

        IUniswapV3PoolState _pool = IUniswapV3PoolState(factory.getPool(token0, token1, fee));
        ...
        uint128 _liquidity0 = LiquidityAmounts.getLiquidityForAmount0(sqrtRatioLX96, sqrtRatioUX96, amount0Desired);

        uint128 _liquidity1 = LiquidityAmounts.getLiquidityForAmount1(sqrtRatioLX96, sqrtRatioUX96, amount1Desired);

        if(_liquidity0 > _liquidity1){
            // swap token0 for token1
            isZeroForOne = true; 
            makingAmount = SqrtPriceMath.getAmount0Delta(sqrtRatioLX96, sqrtRatioCX96,_liquidity0 - _liquidity1 ,true) / 2;
        } else if(_liquidity0 < _liquidity1) {
            // swap token1 for token0
            isZeroForOne = false;
            makingAmount = SqrtPriceMath.getAmount1Delta(sqrtRatioCX96, sqrtRatioUX96, _liquidity1 - _liquidity0,true) / 2;
        } else {
            return(false,0);
        }
    }
